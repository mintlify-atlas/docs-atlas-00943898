...
description: "evaluate.py queries MLflow for the highest-accuracy run, registers it in the Model Registry as spotify-genre-classifier, and assigns the @champion alias for Docker deployment."
---
...
<Steps>
  ...
  <Step title="Register the model version">
    Call `client.create_model_version()` to attach the run's `model` artifact to a named entry in the Model Registry:
  ...
  <Step title="Assign the @champion alias">
    Tag the new version as the champion so downstream consumers can reference it by alias rather than version number:
